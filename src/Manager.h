...
const char UPDATE_FOUND[] PROGMEM         = "<div class=\"container\"><center><div class=\"loader\"id=\"Uloader\"></div></center><div id=\"info\"><b>Update Found</b></div><button onclick=\"update()\">Update</button></div>";
const char SCRIPT[] PROGMEM               = "<script>function clicked(t) {ssid.value= t.innerText; }function btn_click() {var url= \"/add?ssid=\" +ssid.value+ \"&pass=\" +pass.value;call(url,function(d) {document.body.innerHTML= d; }); }function call(url,callback) {var xhr= new XMLHttpRequest();xhr.onreadystatechange= function() {if(this.readyState==4 &&this.status==200) {callback(this.responseText); } };xhr.open(\"GET\",url);xhr.send(); }function check() {Cloader.style.display= \"block\";call(\"/u?update=check\",function(d) {document.body.innerHTML= d; }); }function update() {Uloader.style.display= \"block\";info.style.display= \"none\";call('/u?update=update',function(d) {document.body.innerHTML= d; }); } </script>";
